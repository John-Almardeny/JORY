 ...
 * It represents the Shapes subject and implements all the declared
 * methods in the Subject Abstract Class 
 * @author Yahya Almardeny
 * @version 03/12/2016
 */
public class Shapes extends Subject implements Serializable{
	 ...
	 */
	public Shapes(boolean firstRun) throws FirstRunException{
	...
	public void initialize(){
	    noun.addAll(getShapes());
	    Iterator<ArrayList<String>> it = Resource.getShapes().iterator();
		for(String shape : noun){
			content.put(shape, it.next());
		}
	}
	 ...
	 * returns the basic initial Shapes nouns for the first run as a List
	 * @return shapes
	 */
	private List<String> getShapes(){
		return Arrays.asList(new String[]{"Square","Triangle","Rectangle","Circle","Line", "Star"});
	}
